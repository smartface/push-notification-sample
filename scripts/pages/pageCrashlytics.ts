...
export default class PageCrashlytics extends withDismissAndBackButton(
  PageCrashlyticsDesign
) {
  ...
  InitCrashlytics() {
    try {
      ...
      const customAttribute = new Firebase.analytics.CustomAttribute(
        "newCustomKey",
        "testAnalyticsCustomAttribute"
      );
      setTimeout(
        () => Firebase.analytics.logEvent("testLogEvent", customAttribute),
        5
      );
      ...
      const stringError =
        typeof err === "object" ? JSON.stringify(err, null, "\t") : err;
